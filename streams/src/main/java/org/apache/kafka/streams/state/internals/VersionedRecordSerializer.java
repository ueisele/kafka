...
public class VersionedRecordSerializer<V> implements WrappingNullableSerializer<VersionedRecord<V>, Void, V> {
    private final Serializer<V> valueSerializer;
    private final Serializer<Long> timestampSerializer;

    VersionedRecordSerializer(final Serializer<V> valueSerializer) {
    ...
    public byte[] serialize(final String topic,
                            final VersionedRecord<V> data) {
        if (data == null) {
            return null;
        }
        return serialize(topic, data.value(), data.timestamp(), data.validTo().orElse(Long.MAX_VALUE));
    }
    ...
    public byte[] serialize(final String topic,
        ...
        final byte[] rawValidTo = timestampSerializer.serialize(topic, validTo);
        return ByteBuffer
            .allocate(rawValidTo.length + rawTimestamp.length + rawValue.length)
            .put(rawValidTo)
            .put(rawTimestamp)
            .put(rawValue)
            .array();
    ...
    public void setIfUnset(final SerdeGetter getter) {
        // VersionedRecordSerializer never wraps a null serializer (or configure would throw),
